...
namespace RazorLight
{
	public class EngineHandler : IEngineHandler
	{
		...
		/// <returns></returns>
		public async Task<string> CompileRenderAsync<T>(string key, T model, ExpandoObject viewBag = null, bool isEncoding = false)
		{
			ITemplatePage template = await CompileTemplateAsync(key, isEncoding).ConfigureAwait(false);
			...
			bool isEncoding = false)
		{
			...
			return CompileRenderAsync(key, model, viewBag, isEncoding);
		}
