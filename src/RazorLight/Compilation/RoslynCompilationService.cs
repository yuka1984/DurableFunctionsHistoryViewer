...
namespace RazorLightCustom.Compilation
{
	public class RoslynCompilationService : ICompilationService
	{
	    ...
	    public (Byte[] AssemblyStream, byte[] PdbStream) Compile(IGeneratedRazorTemplate razorTemplate)
	    {
	        ...
	        if (!result.Success)
	        {
	            ...
	            foreach (Diagnostic diagnostic in errorsDiagnostics)
	            {
	                FileLinePositionSpan lineSpan =
	                    diagnostic.Location.SourceTree.GetMappedLineSpan(diagnostic.Location.SourceSpan);
	                string errorMessage = diagnostic.GetMessage();
	                string formattedMessage =
	                    $"- ({lineSpan.StartLinePosition.Line}:{lineSpan.StartLinePosition.Character}) {errorMessage}";
	        ...
	        return (assemblyStream.GetBuffer(), pdbStream.GetBuffer());
	    }

	    public Assembly Emit(byte[] assemblyStream, byte[] pdbStream)
	    {
	        var assembly = Assembly.Load(assemblyStream, pdbStream);
